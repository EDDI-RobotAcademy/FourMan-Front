      ...
      <v-list-item style="height: 20%">
        <v-list-item-content>
            ...
            <div align="center">
              <router-link to="/member-my-page">
                <v-avatar :size="110" color="grey lighten-4">
                  <img
                    src="@/assets/myPage/default_profile_image.png"
                    alt="avatar"
                  />
                </v-avatar>
              </router-link>
          ...
          <v-list-item-subtitle class="black--text justify-center mt-3">
            <div align="center" style="font-size: 17px">{{ nickName }} 님</div>
            <div align="center" class="mt-1" style="font-size: 15px">
              {{ memberType }}
            </div>
            <div
              v-if="memberType == '일반회원'"
              align="center"
              class="mt-1"
...
export default {
  ...
  methods: {
    ...mapActions(myPageModule, ["requestMyInfoForSideBarToSpring"]),
  },
  computed: {
    ...mapState(myPageModule, ["myInfoSideBar"]),
  },
  async created() {
    await this.requestMyInfoForSideBarToSpring(
